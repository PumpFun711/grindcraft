const { generateWorld, BLOCKS, PICKS } = require('./gamedata');

const MAX_PLAYERS_PER_ROOM = 60;
let roomCounter = 1;

class Room {
  constructor(id) {
    this.id = id;
    this.name = `GC-${String(roomCounter++).padStart(2, '0')}`;
    this.players = new Map();
    this.world = generateWorld(40, 30);
    this.cracks = {};
    this.createdAt = Date.now();
  }

  getPlayerCount() { return this.players.size; }
  isFull() { return this.players.size >= MAX_PLAYERS_PER_ROOM; }

  addPlayer(socketId, data) {
    this.players.set(socketId, {
      id: socketId,
      nickname: data.nickname,
      skin: data.skin,
      hair: data.hair,
      shirt: data.shirt,
      pants: data.pants,
      x: data.x || 5,
      y: data.y || 3,
      points: 0,
      totalBlocks: 0,
      pickaxe: 0,
      frame: 0,
      joinedAt: Date.now()
    });
  }

  removePlayer(socketId) { this.players.delete(socketId); }
  getPlayer(socketId) { return this.players.get(socketId); }

  updatePlayerPosition(socketId, x, y) {
    const p = this.players.get(socketId);
    if (p) { p.x = x; p.y = y; }
  }

  getPlayersData() {
    return Array.from(this.players.values());
  }

  mineBlock(socketId, col, row) {
    const player = this.players.get(socketId);
    if (!player) return null;

    const idx = row * 40 + col;
    const blockType = this.world[idx];
    if (blockType < 0) return null;

    const B = BLOCKS[blockType];
    const pick = PICKS[player.pickaxe];
    const hitsNeeded = Math.max(1, Math.ceil(B.baseHits / pick.power));

    const key = `${col},${row}`;
    if (!this.cracks[key]) this.cracks[key] = 0;
    this.cracks[key]++;

    if (this.cracks[key] >= hitsNeeded) {
      this.world[idx] = -1;
      delete this.cracks[key];
      player.points += B.pts;
      player.totalBlocks += 1;
      return {
        broken: true,
        points: B.pts,
        playerPoints: player.points,
        playerBlocks: player.totalBlocks,
        blockType
      };
    } else {
      return {
        broken: false,
        hitsLeft: hitsNeeded - this.cracks[key],
        hitsNeeded
      };
    }
  }

  buyPickaxe(socketId, tier) {
    const player = this.players.get(socketId);
    if (!player) return { success: false, message: 'Player not found' };

    if (tier !== player.pickaxe + 1) {
      return { success: false, message: 'Must upgrade in order' };
    }
    if (tier >= PICKS.length) {
      return { success: false, message: 'Already max tier' };
    }
    const pick = PICKS[tier];
    if (player.points < pick.cost) {
      return { success: false, message: `Need ${pick.cost} pts. You have ${player.points}.` };
    }

    player.points -= pick.cost;
    player.pickaxe = tier;
    return { success: true, newPoints: player.points };
  }

  getLeaderboard() {
    return Array.from(this.players.values())
      .sort((a, b) => b.points - a.points)
      .slice(0, 10)
      .map(p => ({
        nickname: p.nickname,
        points: p.points,
        pickaxe: p.pickaxe,
        totalBlocks: p.totalBlocks
      }));
  }
}

class RoomManager {
  constructor() {
    this.rooms = new Map();
  }

  joinRoom(socketId, playerData) {
    for (const room of this.rooms.values()) {
      if (!room.isFull()) {
        room.addPlayer(socketId, playerData);
        return room;
      }
    }
    const roomId = `room_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const room = new Room(roomId);
    room.addPlayer(socketId, playerData);
    this.rooms.set(roomId, room);
    return room;
  }

  getRoom(roomId) { return this.rooms.get(roomId); }
  removeRoom(roomId) { this.rooms.delete(roomId); }
  getAllRooms() { return Array.from(this.rooms.values()); }

  getRoomStats() {
    return Array.from(this.rooms.values()).map(r => ({
      name: r.name,
      players: r.getPlayerCount()
    }));
  }
}

module.exports = RoomManager;
